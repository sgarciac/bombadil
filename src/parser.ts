...
export class TomlParser extends ct.Parser {
  public documentRule = this.RULE<ast.TopLevelTomlDocumentEntry[]>(
    "documentRule",
    () => {
      ...
      this.MANY(() => {
        this.OR([
          {
            ALT: () => {
              documentEntries.push(this.SUBRULE(this.tableHeaderRule));
            }
          },
          {
            ALT: () => {
              documentEntries.push(
                this.SUBRULE(this.tableArrayEntryHeaderRule)
              );
            }
          },
          {
            ALT: () => {
              documentEntries.push(this.SUBRULE(this.keysValueRule));
            }
          },
          {
            ALT: () => {
              this.CONSUME(l.EndOfLine);
            }
          }
        ]);
      });
  ...
  valueRule = this.RULE("valueRule", () => {
    ...
    this.OR([
      ...
      {
        ALT: () => {
          let image = this.CONSUME(l.Float).image;
          value = ast.tomlAtomicFloat(image, tools.parseFloat(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.TomlInfinity).image;
          value = ast.tomlAtomicInfinity(image, tools.parseInfinity(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.TomlNotANumber).image;
          value = ast.tomlAtomicNotANumber(image, tools.parseNotANumber(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.Integer).image;
          value = ast.tomlAtomicInteger(image, tools.parseDecInteger(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.BinaryInteger).image;
          value = ast.tomlAtomicInteger(image, tools.parseBinaryInteger(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.OctalInteger).image;
          value = ast.tomlAtomicInteger(image, tools.parseOctalInteger(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.HexInteger).image;
          value = ast.tomlAtomicInteger(image, tools.parseHexInteger(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.Booolean).image;
          value = ast.tomlAtomicBoolean(image, tools.parseBoolean(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.OffsetDateTime).image;
          value = ast.tomlAtomicOffsetDateTime(
            image,
            tools.parseOffetDateTime(image)
          );
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.LocalDateTime).image;
          value = ast.tomlAtomicLocalDateTime(
            image,
            tools.parseLocalDateTime(image)
          );
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.LocalDate).image;
          value = ast.tomlAtomicLocalDate(image, tools.parseLocalDate(image));
        }
      },
      {
        ALT: () => {
          let image = this.CONSUME(l.LocalTime).image;
      ...
      {
        ALT: () => {
          value = this.SUBRULE(this.arrayRule);
        }
      },
      {
        ALT: () => {
          value = this.SUBRULE(this.inlineTableRule);
        }
      },
      //Strings
      {
        ALT: () => {
          value = this.SUBRULE(this.literalStringRule);
        }
      },
      {
        ALT: () => {
          value = this.SUBRULE(this.multiLineBasicStringRule);
        }
      },
      {
        ALT: () => {
          value = this.SUBRULE(this.multiLineLiteralStringRule);
        }
      },
      {
        ALT: () => {
          value = this.SUBRULE(this.basicStringRule);
        }
      }
    ]);
    return value;
  ...
  tableArrayEntryHeaderRule = this.RULE(
    "tableArrayEntryHeaderRule",
    (): ast.TomlTableArrayEntryHeader => {
      let headers: string[] = [];
  ...
  basicStringRule = this.RULE("basicStringRule", () => {
    ...
    this.MANY(() => {
      this.OR([
        {
          ALT: () => {
            let image = this.CONSUME(l.EscapedChar).image;
        ...
        {
          ALT: () => {
            let image = this.CONSUME(l.SubBasicString).image;
            basicString += image;
            fullImage += image;
          }
        }
  ...
  literalStringRule = this.RULE(
    "literalStringRule",
    (): ast.TomlAtomicString => {
      let literalString: string = "";
      this.CONSUME(l.OpenLiteralString);
      this.OPTION(() => {
        literalString += this.CONSUME(l.LiteralString).image;
      });
  ...
  multiLineBasicStringRule = this.RULE("multiLineBasicStringRule", () => {
    ...
    this.MANY(() => {
      this.OR([
        ...
        {
          ALT: () => {
            let image = this.CONSUME(l.SubMultiLineBasicString).image;
    ...
    return ast.tomlAtomicString(
      fullImage,
      tools.trimWhiteSpacePrefix(multiLineString)
    );
  });
  ...
  identifierRule = this.RULE(
    ...
    (): string => {
      ...
      this.OR([
        {
          ALT: () => {
            id = this.CONSUME(l.Identifier).image;
          }
        },
        {
          ALT: () => {
            id = this.SUBRULE(this.literalStringRule).value;
          }
        },
        {
          ALT: () => {
            id = this.SUBRULE(this.basicStringRule).value;
          }
        }
      ]);
      return id;
